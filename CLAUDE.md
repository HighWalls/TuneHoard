...
- Download jobs with **live progress streamed over WebSocket** (`/ws/jobs`); 10-second HTTP polling kept as a safety net if the socket drops; reconnect uses exponential backoff (1s → 30s, resets on first message). Cancel kills the subprocess tree. First-run guard: if `library_dir` is unset, the Download button shows a modal pointing the user at Settings instead of letting the request 400.
- Per-job log viewer with `[Expand]` → scrollable 300px pane fetched lazily from `/api/jobs/{id}/log?tail=500` (cached per session), `( ) all  ( ) warnings  ( ) errors` filter radios, and `[ copy ]` to clipboard.
...
- **Per-track audio preview** — `[▶]` play button streams MP3 from `GET /api/audio/{id}` (Range-aware) into a shared `<audio>` element. Inline click-to-seek scrub bar on the playing row, sticky now-playing bar at the bottom (`▶ artist — title  [stop] [vol]`), volume slider persisted to localStorage. Keyboard: Space toggle, ←/→ seek ±5s, Esc stop.
...
- Failed-tracks panel between jobs and library (hidden when count = 0): lists `× artist — title  [open Spotify] [retry] [×]` rows. `[retry]` re-fires the single track as a new job; `[×]` removes the entry from `failures.txt` after a confirm. Header shows "last updated N ago." Polled from `GET /api/failures` on a 30s timer + on focus.
- **Spotify picker** [ Browse Spotify ] button (visible when authorized, auto-shows immediately after Authorize completes) opens a modal listing ★ Liked Songs + ≫ user playlists, with a live search filter; selecting one drops its url into the URL bar. Liked Songs URL preview shows the track count.
...
- Settings page with native file pickers (tkinter), Spotify OAuth (with 5-min auth timeout + scope-mismatch nudge + "authorized N days ago" hint when stale), source reorder, advanced BPM clamp / analysis duration / ffmpeg path inputs, "Update check: N ago — <status>" line.
- Three-button "Scan folder" dialog for indexing pre-existing MP3 collections (read tags only / analyze missing via librosa / abort). The `analyze missing` path runs as a **background scan job** (`POST /api/library/scan?analyze_missing=true` returns a `scan_id` immediately); state is pushed via `/ws/jobs` as `{type: "scan_update"}` and can be cancelled via `DELETE /api/library/scan/{id}` (best-effort — in-flight track finishes).
...
- **OAuth user flow (not Client Credentials).** Spotify tightened Client Credentials access to `playlist_items` in 2025 — it now returns 401 even on public playlists. We use `SpotifyOAuth` with scopes `playlist-read-private playlist-read-collaborative user-library-read`. This reads public + private + collaborative + Liked Songs. **YouTube / SoundCloud URLs don't need any auth at all.** The `user-library-read` scope was added in 2026 for the Spotify picker — pre-existing `.spotify_cache` tokens lack the scope and trigger a one-time re-auth on the next call to `current_user_saved_tracks()`.
- **Third-party Spotify playlists use an anonymous embed fallback.** Development Mode apps get 403 on `playlist_items` for playlists the authorized account doesn't own (2024/2025 API restriction; editorial `37i9dQZF1...` lists 403/404 even on metadata). `get_playlist_tracks` catches that and reads the track list from the public `open.spotify.com/embed/playlist/<id>` page instead (`_embed_playlist_tracks` — same spirit as `applemusic_client`'s anonymous token). Embed rows carry no album/ISRC (search only needs artist + title) and the list is believed to cap ~100 entries — a warning prints when exactly 100 come back. Own playlists still take the API path.
- **One library at a time.** The dashboard targets a single `library_dir` at a time; switching between playlist libraries means changing that path in Settings. Each playlist is a sibling subfolder under `library_dir.parent` with its own `index.csv` and bucket subdirs (the CLI's `--out=parent --into=playlist_name` produces this layout). There is no global cross-playlist index — dedup is intra-playlist.
