...
def api_get_playlists() -> dict[str, Any]:
    """List sibling folders of `library_dir` that contain an `index.csv`.

    Each entry is a self-contained playlist library (mirrors the CLI's
    `--out=parent --into=name` layout: parent/<name>/index.csv +
    parent/<name>/<bucket>/*.mp3).

    Returns 200 even when there's no library configured (so the dashboard
    doesn't have to special-case 4xx); current/parent come back as null in
    that case.
    """
    s = load_settings()
    if not s["library_dir"]:
        return {"current": None, "parent": None, "playlists": []}
    lib = Path(s["library_dir"])
    try:
        lib_resolved = lib.resolve()
    except OSError:
        return {"current": None, "parent": None, "playlists": []}
    if not lib_resolved.parent.exists():
        return {"current": None, "parent": None, "playlists": []}

    parent = lib_resolved.parent
    current_name = lib_resolved.name
    ...
    for sub in children:
        csv_path = sub / "index.csv"
        if not csv_path.is_file():
            continue
        # Lenient: row count = newline count - 1 (header). Don't 500 if the
        # file is mid-write or non-utf8 — show 0 instead.
        track_count = 0
        try:
            with csv_path.open("r", encoding="utf-8", newline="") as f:
                # Subtract 1 for the header row. Empty file → 0.
                line_count = sum(1 for _ in f)
            track_count = max(0, line_count - 1)
        except Exception:
            track_count = 0
        entries.append({
            "name": sub.name,
            "track_count": track_count,
            "is_current": sub.name == current_name,
        })
...
class PlaylistSwitch(BaseModel):
    name: str


@app.post("/api/library/switch")
def api_switch_library(req: PlaylistSwitch) -> dict[str, Any]:
    """Switch `library_dir` to a sibling folder by name. Validates the target
    is a real subfolder of `library_dir.parent` and contains an `index.csv`.

    Path-traversal protection: reject names containing path separators or
    parent-dir tokens. After resolving the candidate, verify its parent is
    EXACTLY the current parent — defends against symlinks / case-only matches
    that pass the cheap string check."""
    s = load_settings()
    if not s["library_dir"]:
        raise HTTPException(400, "library_dir not configured")
    ...
    # Cheap traversal guard: no separators, no "..", no leading dot tricks.
    ...
    # Belt-and-suspenders: candidate must live directly under parent.
    if candidate.parent != parent:
        raise HTTPException(400, "invalid playlist name")
    if not candidate.is_dir():
        raise HTTPException(400, f"playlist folder not found: {name}")
    if not (candidate / "index.csv").is_file():
        raise HTTPException(400, f"no index.csv in playlist folder: {name}")

    s["library_dir"] = str(candidate)
    save_settings(s)
    return api_get_settings()
